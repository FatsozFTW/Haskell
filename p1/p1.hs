...
-- Exercise 1
evalErr :: CALC -> Int
evalErr (Num x) = if x<0 then error "No negative plz" else x
evalErr (Plus l r) = (evalErr l) + (evalErr r)
evalErr (Minus l r) = let x  = ((evalErr l) - (evalErr r)) in
    if x <0
    then error "Sub is neg"
    else x
evalErr (Mult l r) = (evalErr l) * (evalErr r)
evalErr (Div l r) = if (evalErr r) == 0 then error "Can't div by zero" else (evalErr l) `div` (evalErr r)
...
interpCALC strxng = evalM(parseCALC strxng) -- add dot maybe
